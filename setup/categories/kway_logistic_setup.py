...
def get_KLclassifier_config(args, model, domain):
    ...
    # Set up the model
    klmodel_without_ddp = KWayLogisticWrapper(model)
    klmodel = None
    ...
    if distrib.is_dist_avail_and_initialized():
        ...
        if len(args.gpulist) > 1:
            ...
            klmodel = torch.nn.parallel.DistributedDataParallel(klmodel_without_ddp, device_ids=None, output_device=None)
        else:
            ...
            klmodel = torch.nn.parallel.DistributedDataParallel(klmodel_without_ddp, device_ids=[args.gpulist[0]])
    else:
        if len(args.gpulist) == 1:
            ...
            klmodel = klmodel_without_ddp.to(args.gpulist[0])

#    if (distrib.is_main_process()  and not args.no_wandb):
#        wandb.watch(klmodel_without_ddp)
    ...
    config.model_without_ddp = klmodel_without_ddp
    ...
    if hasattr(klmodel_without_ddp, 'train_config'):
        model_train_config = klmodel_without_ddp.train_config()
