                        ...
                        if(args.interpolate_shift):
                            ...
                            if not os.path.isfile(autoencoder_hbest_path+".done"):
                                ...
                                for epoch in range(1, autoencoder_config.max_epoch+1):
                                    ...
                                    # Why do this by hand instead of using iterative_trainer?
                                    # Because iterative_trainer seems to leak GPU memory via keeping losses.
                                    # Which is especially bad when your loss graph references both the input and output, like in an AE 
                                    # This was getting crippling by the second stage
                                    ...
                                    for i, (sample, label) in enumerate(autoencoder_config.valid_loader):
                                       x = sample.to(args.device)
                            ...
                            for i, (sample, label) in enumerate(d2_test_base):
                                # add extra dimension for batch_rate 1
                                x = sample[None]
                                x = x.to(args.device)
                            ...
                            for i in range(final_len):
                                ...
                                for j in range(len(decoded[0])):
                                    d2_test.append(decoded[j].cpu(),d2_test_latent[i][1])
